...
| ~~23~~ | **SHIPPED** — risk requirement to client with hard deadline (R18/D13) | `riesgo_requerimientos` (migration `1700004700000`), deadline = ETA + offload window, `services/requerimientosService.ts` + `routes/riesgoRequerimientos.ts`, expiry sweep as **phase 3 of the tick** firing CT-4 → hold tipo 'riesgo'. The clock only ever runs against a client who was actually notified (`notificado_at IS NOT NULL`), so an unprovisioned SMTP cannot freeze somebody's cargo. Code side of #22 (`services/mailer.ts`) shipped with it; the app password is still a USER task below. |
| ~~26~~ | **SHIPPED** — contingency engine CT-1…CT-7 | pure engine `shared/operaciones/replan.ts` (ruleset `2026-08a` + sha256 hash, `evaluarContingencias`), migration `1700004800000_replan.ts` (`replan_evaluaciones` stores the exact snapshot = replayable decision; `replan_acciones` holds the pending proposals, UNIQUE on `(operacion_id, clave)` so the ledger cannot stutter), `services/replanService.ts`, `routes/replan.ts`, phase 4 of the tick. Auto-executes exclude/reschedule/open-hold/suspend-units/notify; **proposes** the money-touching reassignment, confirmed or discarded by a human with an obligatory `motivo` written as `override = true`. Two honest gaps, both documented in code: `notificar` records that an advice is OWED (no SMTP, #22/#31), and CT-7 falls back to `estado_planeacion = 'asignada'` until `despachos` exists (#29) |
