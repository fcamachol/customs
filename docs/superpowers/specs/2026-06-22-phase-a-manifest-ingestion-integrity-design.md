...
**Idempotency key (per-line, not per-shipment):** the plain composite string `${mawbReference}|${guideId}|${lineSeq}|${hsCode}` where `lineSeq` = 1-based ordinal of the line within its `guideId` group. Rationale: the real file has 501 rows but only 126 unique `guideId`/`clientOrderId` — keying without a per-line discriminator would collapse ~375 partidas and the UNIQUE constraint would drop them. The golden test asserts **501 distinct staging rows**. **Scope note (accuracy):** Phase A delivers the idempotency-key infrastructure plus **within-manifest** idempotent promotion — re-promoting a manifest UPSERTs by key and never duplicates partidas, and a second promote is state-guarded (409). **Cross-upload merge** (re-uploading a corrected file *into the same manifest* rather than creating a new one) is **deferred to Increment 2**: today each `POST /` creates a new manifest, so a corrected re-upload yields a new manifest version, not a merge into the prior one.
...
- Integration (`server/test/routes/manifests.test.ts` — rewrite for multipart): partial-accept response shape; quarantine excluded from promotion; within-manifest promote UPSERT idempotency (re-promote never duplicates; second promote → 409); promote blocked while errors remain / on `promoted` state / zero valid rows; re-promotion sets `risk_stale`; 409 on locked manifest; audit events emitted.
