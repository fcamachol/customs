...
describe('mergeDistribution', () => {
  it('fills missing colors with zero', () => {
    expect(mergeDistribution([{ risk_color: 'verde', n: 5 }, { risk_color: 'rojo', n: 2 }]))
      .toEqual({ verde: 5, amarillo: 0, rojo: 2, gris: 0 });
  });
});
...
describe('buildDashboardResponse', () => {
  it('omits byUser when no byUserRows', () => {
    ...
    expect(r).toEqual({ manifests: 3, distribution: { verde: 9, amarillo: 0, rojo: 0, gris: 0 } });
  ...
  it('groups per-user distributions and manifest counts', () => {
    ...
    expect(r.byUser).toEqual([
      { userId: 'u1', username: 'Ana', manifests: 1, distribution: { verde: 3, amarillo: 0, rojo: 1, gris: 0 } },
      { userId: 'u2', username: 'Beto', manifests: 1, distribution: { verde: 1, amarillo: 0, rojo: 0, gris: 0 } },
    ]);
  });
});
