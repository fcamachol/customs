...
describe('hallazgoHash', () => {
  ...
  it('las once señales tienen criterio de huella declarado', () => {
    ...
    expect(Object.keys(HUELLA_EVIDENCIA).sort()).toEqual(
      ['agregado', 'bbdd', 'cantidad', 'clasificacion_inconsistente', 'denied_party',
       'descripcion_generica', 'direcciones', 'id', 'monto', 'pirateria', 'prohibidos'],
    );
  });
});
...
describe('colorEfectivo', () => {
  it('sin supresiones devuelve exactamente lo que dijo el motor', () => {
    ...
    // 35 pts crudos sobre maxPoints=398 → 9. El score se movió (10 con 348, 9 con 373 y con 398)
    // porque cambió el denominador, no porque cambiara el juicio. La banda, que es lo que el
    // operador lee, sigue siendo amarillo en las tres versiones — para eso se ajustan los cortes
    // junto con cada peso nuevo. ESTE aserto es el que atrapó que bajar `rojo` a 9 en 2026-09b
    // habría convertido esta combinación vieja en roja; por eso `rojo` se quedó en 10.
