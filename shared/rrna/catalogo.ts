/**
 * shared/rrna/catalogo.ts — Regulaciones y Restricciones No Arancelarias.
 *
 * ESTE CATÁLOGO YA EXISTÍA, completo y documentado, en `src/constants/rrnaCategories.ts`, y
 * **ningún archivo lo importaba**. Diez categorías con su autoridad y su fundamento (RGCE 3.7.5-E:
 * esta mercancía debe despacharse por pedimento A1 con agente aduanal y padrón, no por T1), y ni
 * una línea que las evaluara. Es el cuarto caso del mismo patrón en este repo —después de la
 * descripción, la fracción y la clave de aduana—: dato o regla capturada y descartada.
 *
 * Se movió a `shared/` sin tocar su contenido para que el SERVIDOR pueda usarlo: vivía bajo `src/`,
 * que es sólo el front. `src/constants/rrnaCategories.ts` quedó como re-export, así que cualquier
 * import futuro por la ruta vieja sigue resolviendo al mismo objeto y no puede haber dos catálogos.
 *
 * Qué NO hace este módulo, a propósito: no alimenta el semáforo. Una coincidencia aquí es
 * INFORMATIVA —"esta descripción se parece a algo que requiere permiso"— y no una acusación:
 * la lista es por palabra clave, así que "aceite" pega igual en aceite comestible que en aceite
 * para bicicleta. Meterla al score obligaría a recalibrar las bandas y, peor, convertiría una
 * heurística de texto en una banda roja. Se reporta aparte y quien revisa decide.
 */
